<template>
    <div class="flex justify-between items-center bg-zinc-900 py-5 px-6">
        ...
        <div class="w-2/5 text-center mb-2 md:mb-0 flex items-center justify-center">
            ...
            <button class="ml-3 mr-2 px-3 py-2 rounded-xl transition-colors border-solid border-zinc-700 border" @click="switchMediaType(MediaType.ANIME)"
            ...
            <button class="px-3 py-2 rounded-xl transition-colors border-solid border-zinc-700 border" @click="switchMediaType(MediaType.MANGA)"
    ...
    <!--<div class="py-5 px-6 block text-center md:flex md:justify-between md:items-center shadow-sm">
        <div>
            <div v-if="viewerStore.token === '' || viewerStore.viewer == undefined">
                <a href="https://anilist.co/api/v2/oauth/authorize?client_id=5767&response_type=token">Login to Anilist</a>
            </div>
            <div v-else>
                <a :href="userLink" target="_blank" class="justify-center md:justify-start mb-3 md:mb-0 flex items-center space-x-3">
                    <img :src="viewerStore.viewer.avatar?.medium" width="48" height="48" class="rounded-full">
                    <p class="text-xl font-bold mb-2 md:mb-0 hover:text-slate-300 transition-colors">{{ viewerStore.viewer.name }}</p>
                </a>
            </div>
        </div>
